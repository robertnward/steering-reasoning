...
class Qwen2LastLayer(Qwen2DecoderLayer):
    ...
    def __call__(
        ...
        elif self.proj_type in ["all_attn", "all_layer", "base"]:
            q = q_vanilla
            k = k_vanilla
            v = v_vanilla
        ...
        # Fully Connected
        if self.proj_type in ["all_layer", "base"]:
        ...
        if self.proj_type == "all_layer":
            hidden_states = hidden_states + self.steering_vector

        return hidden_states, residual


class Qwen2ForCausalLM_CustomLast(Qwen2ForCausalLM):
