...
class RayGroup:
    ...
    def prepare_reference_model(self, accelerator, is_deepspeed_enabled):
        return [
            actor.prepare_reference_model.remote(accelerator, is_deepspeed_enabled)
            for actor in self._actor_handlers
        ]

    def reference_forward(self, records: dict[str, torch.Tensor], index: int):
        assert index < len(self._actor_handlers), (
        ...
        **kwargs,
        ...
        for i, actor in enumerate(self._actor_handlers):
            ref = actor.run.remote(
                config=config, vllm_engines=vllm_engines, *args, **kwargs
            )
            refs.append(ref)
